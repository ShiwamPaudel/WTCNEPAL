  ...
  useEffect(() => {
    let getProduct = async () => {
      let product = await axios.get(
        `${BaseUrl}/products?populate=*&pagination[start]=0&pagination[limit]=${limit}`
      );
        ...
        <div className="container 2xl:max-w-[1180px] xl:px-20 2xl:px-0   mx-auto py-10 px-[15px] md:px-0">
          ...
          <div className="flex items-center justify-center ">
            ...
            {isLoading3 ? (
              ...
              <button
                onClick={handleLimit}
                className="h-[50px] w-[130px] bg-[#23A8CD] flex items-center justify-center text-white text-[16px] rounded-[4px]"
              >
                Load More
              </button>
            )}
