...
const AboutUs = () => {
  ...
  useEffect(() => {
    let getAbout = async () => {
      try {
        // Fixed typo: 'banenr' -> 'banner'
        let banner = await axios.get(`${BaseUrl}/customers?populate=*`);
        setCustomer(banner.data.data);
      } catch (error) {
  ...
  return (
    <div className="custom-bg">
      <div className="bg-gradient-to-r from-black/50 to-blue-600/95 md:from-neutral-50/90 md:to-blue-600/95 py-10">
        {/* ADDED key={item.id} below */}
        {customer?.map((item: any, index: number) => (
          <div key={item.id || index} className="md:flex my-8 items-center">
