...
const ProductDetail = ({ product, error }: ProductDetailProps) => {
    ...
    if (error || !product) {
        ...
        const extractedId = extractIdFromSlug(currentSlug || '');
        return (
    ...
    const breadcrumbs = [
        ...
        },
        {
            ...
            item: `https://wtcnepal.com/product-single/${router.query.slug}`,
        },
    ];
    ...
    const productImg = product?.attributes?.image?.data?.attributes?.url ? imageUrl(product.attributes.image.data.attributes.url) : "/assets/logo.png";
    const canonicalUrl = `https://wtcnepal.com/product-single/${router.query.slug}`;
