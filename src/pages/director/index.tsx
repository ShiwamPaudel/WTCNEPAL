...
const index = () => {
  ...
  useEffect(()=>{
    ...
    let getTestimonial =async()=>{
      let response = await axios.get(`${BaseUrl}/meet-our-directors?populate=*`)
      ...
      testimonial?.map((item:any,index:number) =>(
        ...
        <div className="basis-[60%]">
          <div className="flex items-center justify-start ">
            <h1 className="text-[28px] font-semibold "> {item?.attributes?.title}</h1>
          </div>
          ...
          <div className="pt-[10px]">
            <p className="font-semibold text-[16px]"> {item?.attributes?.name}</p>
            <p>{item?.attributes?.position}</p>
            <p>{item?.attributes?.company_name}</p>
          </div>
        </div>
      </div>
      )) :<div className="max-w-[1180px] mx-auto flex justify-center items-center" style={{ zIndex: 100,backgroundColor:'white' }}>
